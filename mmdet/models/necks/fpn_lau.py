...
class FPN_LAU(BaseModule):
    def __init__(self,
        ...
        super(FPN_LAU, self).__init__(init_cfg)
        ...
        assert self.upsample in [
            'nearest', 'bilinear', 'deconv', 'pixel_shuffle', 'carafe', 'lda_aqu', None
        ]
        ...
        for i in range(self.start_level, self.backbone_end_level):
            ...
            if i != self.backbone_end_level - 1:
                if self.upsample == 'lda_aqu':
                    upsample_module = LDA_AQU(in_channels=out_channels, reduction_factor=self.upsample_cfg.reduction_factor, k_e=self.upsample_cfg.k_e, k_u=self.upsample_cfg.k_u, n_groups=self.upsample_cfg.n_groups, range_factor=self.upsample_cfg.range_factor)
                else:
    ...
    def init_weights(self):
        ...
        super(FPN_LAU, self).init_weights()
        for m in self.modules():
